...
PARTITIONER=$2
    ...
    then
        # Run SUT
        mpirun -np ${NUMRANKS} -map-by numa:PE=2 ${sut} --verbose --partitioner $PARTITIONER --output-partition $partFile --model-options "--topo=torus --shape=4x4x4 --cmdLine=\"Init\" --cmdLine=\"Allreduce\" --cmdLine=\"Fini\"" ${sutArgs} > $outFile 2>$errFile
        RetVal=$?
