...
def generate_scheduler_inputs (InfoPair, TimeObject, JobObjects, options):
    ...
    for Job in JobObjects:
        if Job.jobNum not in completedJobsList:
            ...
            for l in mFile:
	        #sys.stderr.write(" %d  l= %s\n" % (Job.jobNum), (l))
	        #sys.stderr.write(" %d" % (Job.jobNum))
                sys.stderr.write(str(Job.jobNum))
	        sys.stderr.write(l)
                emberMotifNum += 1
            #if Job.jobNum == 3 and emberMotifNum == 13 :
            if Job.jobNum == 3 :
                johnsF = 1
            if johnsF == 1 :
            ...
            if (emberMotifNum == -1):
                print ("ExxROR: No lines in the motifLog file  for Job " + str(Job.jobNum))
                sys.stderr.write("ERROR: No lines in the motifLog file  for Job " + str(Job.jobNum) + "\n")
                print (str(Job.jobNum))
                sys.exit(1)
