...
getconfig() {
    ...
    # Build default dependencies: ramulator, GoblinHMCSim, DramSim3, no cuda
    ...
    case $1 in
        ...
        sstmainline_config_no_mpi)
            ...
            coreMiscEnv="${cc_environment}"
            elementsMiscEnv="${cc_environment}"
            depsStr="-r default -R stabledevel -G default -D default -A none"
            setConvenienceVars "$depsStr"
            coreConfigStr="$corebaseoptions $coreMiscEnv --disable-mpi"
            elementsConfigStr="$elementsbaseoptions --with-ramulator=$SST_DEPS_INSTALL_RAMULATOR --with-ramulator2=$SST_DEPS_INSTALL_RAMULATOR2 --with-goblin-hmcsim=$SST_DEPS_INSTALL_GOBLIN_HMCSIM --with-dramsim3=$SST_DEPS_INSTALL_DRAMSIM3 --with-pin=$SST_DEPS_INSTALL_INTEL_PIN $elementsMiscEnv --with-otf2=$SST_DEPS_INSTALL_OTF2"
            macroConfigStr="${NOBUILD}"
            externalelementConfigStr="$externalelementbaseoptions"
            junoConfigStr="$junobaseoptions"
            ;;
        ...
        sstmainline_config_macosx_no_gem5)
            ...
            setConvenienceVars "$depsStr"
            coreConfigStr="$corebaseoptions $coreMiscEnv"
            elementsConfigStr="$elementsbaseoptions --with-ramulator=$SST_DEPS_INSTALL_RAMULATOR --with-goblin-hmcsim=$SST_DEPS_INSTALL_GOBLIN_HMCSIM --with-dramsim3=$SST_DEPS_INSTALL_DRAMSIM3 $elementsMiscEnv"
            macroConfigStr="${NOBUILD}"
            externalelementConfigStr="$externalelementbaseoptions"
            junoConfigStr="$junobaseoptions"
            ;;
    ...
    case ${build_type} in
        sstmainline_config|sstmainline_coreonly_config|sstmainline_coreonly_config_no_mpi|sstmainline_config_no_gem5|sstmainline_config_clang_core_only|sstmainline_config_macosx_no_gem5|sstmainline_config_no_mpi|sstmainline_config_make_dist_test|sstmainline_config_core_make_dist_test|documentation|sstmainline_config_all|sstmainline_config_linux_with_cuda|sstmainline_config_linux_with_cuda_no_mpi|sst-macro_withsstcore_mac|sst-macro_nosstcore_mac|sst-macro_withsstcore_linux|sst-macro_nosstcore_linux|sst_Macro_make_dist)
            ...
            if [[  ${SST_WITHOUT_PIN:+isSet} == isSet ]] ; then
                ...
                if ModuleEx avail | grep -E -q "pin/pin-3.31"; then
                    if [ $kernel != "Darwin" ] ; then
                        echo "LOADING INTEL PIN ENVIRONMENT MODULE"
                        ModuleEx load pin/pin-3.31
