# !/bin/bash
# sstDep_gem5_stabledevel_static.sh
...
export SST_BUILD_GEM5_STABLEDEVEL_STATIC=1
#===============================================================================
# GEM5
#===============================================================================
#-------------------------------------------------------------------------------
...
#     None
#-------------------------------------------------------------------------------
export SST_DEPS_GEM5_VERSION_LOCAL="STABLEDEVEL STATIC"
export SST_DEPS_SRC_STAGED_GEM5=${SST_DEPS_SRC_STAGING}/sst-gem5-devel.devel
sstDepsStage_gem5 ()
{
    sstDepsAnnounce -h $FUNCNAME -m "Staging sstgem5 stabledevel_static"
    ...
    do
        ...
        then
            ...
            then
                ...
                echo "ERROR: sstDep_gem5_stabledevel_static.sh: sstmacro hg repository clone failure."
...
sstDepsDeploy_gem5 ()
{
    sstDepsAnnounce -h $FUNCNAME -m "Deploying sstgem5 stabledevel_static"
    ...
    then
        ...
        scons build/X86_SE/libgem5_opt.a --verbose < $TMPDIR/yes.in
    else
        yes "" | scons -j4 build/X86_SE/libgem5_opt.a --verbose
    fi
    #unset CC
    #unset CXX
    # ------------------------------------------------------------------------
    ...
    then
        # bail out on error
        echo "ERROR: sstDep_gem5_stabledevel_static.sh: gem5 scons failure"
        return $retval
    else
	sstDepsAnnounce -h $FUNCNAME -m "SCONS (STATIC) SUCCESSFUL"
    fi

    # NOTE: There should be a "make install", but due to
    # idiosyncrasies of gem5-sst integration, this is how it is
    # installed
    sstDepsAnnounce -h $FUNCNAME -m "Installing sstgem5 stabledevel_static"
