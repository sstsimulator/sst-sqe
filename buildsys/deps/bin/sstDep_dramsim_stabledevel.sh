...
export SST_BUILD_DRAMSIM_STABLEDEVEL=1
#===============================================================================
# DRAMSim
#===============================================================================
#-------------------------------------------------------------------------------
...
sstDepsStage_dramsim ()
{
    sstDepsAnnounce -h $FUNCNAME -m "Staging DRAMSim stabledevel"
    ...
    then
        # bail out on error
        echo "ERROR: sstDep_dramsim_stabledevel.sh: DRAMSim repository fetch failure"
    ...
    sstDepsAnnounce -h $FUNCNAME -m "Fetched DRAMSim2 $dramsimHeadRevision"
...
sstDepsDeploy_dramsim ()
{
    sstDepsAnnounce -h $FUNCNAME -m "Deploying DRAMSim stabledevel"
    ...
    then
        # bail out on error
        echo "ERROR: sstDep_dramsim_stabledevel.sh: libdramsim make failure"
        return $retval
    fi
