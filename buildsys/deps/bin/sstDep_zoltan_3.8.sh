...
#-------------------------------------------------------------------------------
export SST_DEPS_SRC_STAGED_ZOLTAN=${SST_DEPS_SRC_STAGING}/Zoltan_v3.8
sstDepsStage_zoltan ()
{
    sstDepsAnnounce -h $FUNCNAME -m "Staging Zoltan 3.8"

    # Extract tarfile. Once unpacked, files should be available in
    # $SST_DEPS_SRC_STAGED_ZOLTAN
    tar xfz ${SST_DEPS_SRC_PRISTINE}/zoltan_distrib_v3.8.tar.gz -C ${SST_DEPS_SRC_STAGING} 2> errFile
    retval=$?
    touch errFile
    if [ -s errFile ] ; then
    ...
    then
        # bail out on error
        echo "ERROR: sstDep_zoltan_3.8.sh: Zoltan untar failure"
        return $retval
    fi
...
sstDepsDeploy_zoltan ()
{
    sstDepsAnnounce -h $FUNCNAME -m "Deploying Zoltan 3.8"
    ...
    then
        # bail out on error
        echo "ERROR: sstDep_zoltan_3.8.sh: zoltan configure failure"
        return $retval
    fi
    ...
    then
        # bail out on error
        echo "ERROR: sstDep_zoltan_3.8.sh: Zoltan make everything failure"
    ...
    then
        # bail out on error
        echo "ERROR: sstDep_zoltan_3.8.sh: Zoltan make install failure"
        return $retval
    fi
...
sstDepsQuery_zoltan ()
{
    ...
    echo "export SST_DEPS_VERSION_ZOLTAN=\"3.8\""
