...
export SST_BUILD_QSIM_SST_2_3=1
#===============================================================================
# Qsim
#===============================================================================
#-------------------------------------------------------------------------------
...
#     None
#-------------------------------------------------------------------------------
export SST_DEPS_SRC_STAGED_QSIM=${SST_DEPS_SRC_STAGING}/Qsim_SST-2.3
sstDepsStage_qsim ()
{
    sstDepsAnnounce -h $FUNCNAME -m "Staging Qsim SST-2.3"

    # fetch latest from Qsim repository
    pushd ${SST_DEPS_SRC_STAGING}
    svn co svn://s917191.sandia.gov/qsim/tags/SST-2.3 Qsim_SST-2.3

    retval=$?
    if [ $retval -ne 0 ]
    then
        # bail out on error
        echo "ERROR: sstDep_qsim_SST-2.3.sh: Qsim repository fetch failure"
        sstDepsAnnounce -h $FUNCNAME -m "Is the SVN http proxy configured properly?"
        popd
        return $retval
    fi

    cp ${SST_DEPS_SRC_PRISTINE}/qemu-0.12.3.tar.gz  Qsim_SST-2.3
    cp Qsim_SST-2.3/remote/client/qsim-client.h     Qsim_SST-2.3
    cp Qsim_SST-2.3/remote/qsim-net.h               Qsim_SST-2.3
...
sstDepsDeploy_qsim ()
{
    sstDepsAnnounce -h $FUNCNAME -m "Deploying Qsim SST-2.3"
    ...
    then
        # bail out on error
        echo "ERROR: sstDep_qsim_SST-2.3.sh: qsim make install failure"
        popd
        return $retval
    fi

    sstDepsAnnounce -h $FUNCNAME -m "Installing qsim SST-2.3"
...
export SST_DEPS_INSTALL_QSIM=${SST_DEPS_SRC_STAGING}/Qsim_SST-2.3
#-------------------------------------------------------------------------------
# Function:
#     sstDepsQuery_qsim
# Purpose:
