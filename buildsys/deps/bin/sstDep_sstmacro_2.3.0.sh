...
export SST_BUILD_SSTMACRO_2_3_0=1
#===============================================================================
# SSTMACRO
#===============================================================================
#-------------------------------------------------------------------------------
...
#     None
#-------------------------------------------------------------------------------
export SST_DEPS_SRC_STAGED_SSTMACRO_2_3_0=${SST_DEPS_SRC_STAGING}/sstmacro-2.3.0
sstDepsStage_sstmacro ()
{
    sstDepsAnnounce -h $FUNCNAME -m "Staging sstmacro 2.3.0"

    # Extract tarfile. Once unpacked, files should be available in
    # $SST_DEPS_SRC_STAGED_SSTMACRO
    tar xfz ${SST_DEPS_SRC_PRISTINE}/sstmacro-2.3.0.tar.gz -C ${SST_DEPS_SRC_STAGING}
    retval=$?
    if [ $retval -ne 0 ]
    then
        # bail out on error
        echo "ERROR: sstDep_sstmacro_2.3.0.sh: sstmacro untar failure"
        return $retval
    fi
...
sstDepsDeploy_sstmacro ()
{
    sstDepsAnnounce -h $FUNCNAME -m "Deploying sstmacro 2.3.0"
    pushd ${SST_DEPS_SRC_STAGED_SSTMACRO_2_3_0}
    ...
    then
        # bail out on error
        echo "ERROR: sstDep_sstmacro_2.3.0.sh: sstmacro bootstrap.sh failure"
        return $retval
    fi
    ...
    then
        # bail out on error
        echo "ERROR: sstDep_sstmacro_2.3.0.sh: sstmacro configure failure"
    ...
    then
        # bail out on error
        echo "ERROR: sstDep_sstmacro_2.3.0.sh: sstmacro make failure"
        echo "       sstmacro requires boost. Is Boost installed?"
        return $retval
    fi
    ...
    then
        # bail out on error
        echo "ERROR: sstDep_sstmacro_2.3.0.sh: sstmacro install failure"
        return $retval
    fi
...
sstDepsQuery_sstmacro ()
{
    ...
    echo "export SST_DEPS_VERSION_SSTMACRO=\"2.3.0\""
