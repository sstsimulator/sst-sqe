...
#     None
#-------------------------------------------------------------------------------
export SST_DEPS_SRC_STAGED_BOOST=${SST_DEPS_SRC_STAGING}/boost_1_49_0
sstDepsStage_boost ()
{
    sstDepsAnnounce -h $FUNCNAME -m "Staging Boost 1.49"

    # Extract tarfile. Once unpacked, files should be available in
    # $SST_DEPS_SRC_STAGED_BOOST
    tar xfz ${SST_DEPS_SRC_PRISTINE}/boost_1_49_0.tar.gz -C ${SST_DEPS_SRC_STAGING}
    retval=$?
    if [ $retval -ne 0 ]
    then
        # bail out on error
        echo "ERROR: sstDep_boost_1.49.sh: Boost untar failure"
        return $retval
    fi
...
sstDepsDeploy_boost ()
{
    sstDepsAnnounce -h $FUNCNAME -m "Deploying Boost 1.49"
...
' project-config.jam
    ...
    then
        # bail out on error
        echo "ERROR: sstDep_boost_1.49.sh: Boost project-config.jam modification failure"
        return $retval
    fi
    ...
    then
        # bail out on error
        echo "ERROR: sstDep_boost_1.49.sh: Boost deployment failure (bjam)"
        return $retval
    fi
...
sstDepsQuery_boost ()
{
    ...
    echo "export SST_DEPS_VERSION_BOOST=\"1.49\""
