...
#===============================================================================
    ...
    if [[ ${SST_MULTI_THREAD_COUNT:+isSet} == isSet ]] ; then
       if [ $SST_MULTI_THREAD_COUNT -gt 1 ] ; then
           echo '           SKIP '
           preFail "Iris does not work with threading" "skip"
       fi
    fi     
...
echo ARGS `/bin/ls -l ${sutArgs}`
        ...
        then
             ...
             grep -v 'Warning:.*undoc' $errFile > _errFile
             echo "The Error File (omitting undoc warnings)  first 10 and last 10 lines:"
             sed 10q _errFile
             echo "       - - -"
             tail -10 _errFile
             echo ' '
