...
#===============================================================================
L_SUITENAME="SST_SweepEmber_suite" # Name of this test suite; will be used to
...
# Purpose:
#     Exercise the SweepEmber code in SST
# Inputs:
#     None
# Outputs:
#     test_SweepEmber.out file
# Expected Results
...
SE_start() {
    ...
    echo " Test $TEST_INDEX, $FAILED_TESTS have failed"
    startSeconds=`date +%s`
    FAILED="FALSE"
    PARAMS="$1"
    echo "     $1"
...
SE_fini() {
   ...
   if [ $? != 0 ] ; then 
      ...
      fail "       SST run is incomplete, FATAL" 
      FAILED="TRUE"
   else
       echo $TL
       echo $1   $TL >> $SST_TEST_OUTPUTS/SweepEmber_cumulative.out
       RL=`grep $1 $SST_TEST_REFERENCE/test_SweepEmber.out`
       if [ $? != 0 ] ; then 
          echo " Can't locate this test in Reference file "
          fail " Can't locate this test in Reference file "
          FAILED="TRUE"
       else
           if [[ "$RL" != *"$TL"* ]] ; then 
               ...
               echo "Out Put   $TL" 
               fail "output does not match reference time"
               FAILED="TRUE"
           fi
       fi
   fi
